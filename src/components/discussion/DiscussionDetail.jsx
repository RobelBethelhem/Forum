...
function ReplyItem({ reply, parentId, parentUsername, onLikeReply, onReplyToReply, userVotes }) {
  ...
  return (
    <div style={{ display: 'flex', marginBottom: 16, alignItems: 'flex-start', flexWrap: 'wrap' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ marginBottom: 8, wordBreak: 'break-word', fontSize: 15 }}>
          <span style={{ color: '#007bff', fontWeight: 500, marginRight: 4 }}>@{parentUsername}:</span>{reply.content}
        </div>
...
// Helper to flatten all replies for a comment, with parentUsername
function flattenReplies(replies, parentUsername) {
  const flat = [];
  function traverse(list, parentName) {
    list.forEach(r => {
      flat.push({ ...r, parentUsername: parentName });
      if (r.replies && r.replies.length > 0) {
        traverse(r.replies, r.User?.username || 'Unknown');
      }
    });
  }
  traverse(replies, parentUsername);
  return flat;
}
...
function CommentItem({ comment, onLike, onReply, onLikeReply, onReplyToReply, userVotes }) {
  ...
  const flatReplies = flattenReplies(comment.replies || [], comment.User?.username || 'Unknown');
  ...
  return (
    <div style={{ display: 'flex', marginBottom: 16, alignItems: 'flex-start', flexWrap: 'wrap' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ marginBottom: 8, wordBreak: 'break-word', fontSize: 15 }}>{comment.content}</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, fontSize: 14, flexWrap: 'wrap' }}>
        ...
        {flatReplies.length > 0 && !showReplies && (
          <div style={{ marginLeft: 40, marginBottom: 16 }}>
            <button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }} onClick={() => setShowReplies(true)}>View {flatReplies.length} {flatReplies.length === 1 ? 'Reply' : 'Replies'}</button>
          </div>
        )}
